...
import { createAxiosClient, createAxiosJwt } from "src/helper/axiosInstance";
...
const ProductAssetCreate = () => {
  ...
  const assetsToRender = () => {
    if (
      !store.list.loading &&
      store.list.result &&
      store.list.result?.assets?.length
    ) {
      ...
      >
        <Row>
          ...
          <Col span={24}>
            <Row
              gutter={[16, 16]}
              style={{ height: 500, overflowY: "scroll", padding: "20px 0" }}
            >
              {assetsToRender()}
            </Row>
            <div
